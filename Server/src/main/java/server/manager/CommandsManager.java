...
import server.deserializers.ArgumentsDeserializer;
...
public class CommandsManager {
    ...
    Logger log = LoggerFactory.getLogger(CommandsManager.class);
    ...
    public void sendCommandInfo(Scannable serverReader, Printable serverPrinter) throws Exception {
        ...
        try {
            ...
            log.error("Ошибка при отправке информации о команде");
            selfPrint.printLine("Что-то пошло не так: " + e.getMessage());
        }
        log.info("Ответ на выполнение запроса " + commandName + " отправлен клиенту");
    ...
    public CommandInfo getCommandInfo(String commandName) {
        switch (commandName) {
            case "clear": {
                log.info("Сервер получил команду clear");
                return new CommandInfo(0, 0, commandName);
            }

            case "show": {
                log.info("Сервер получил команду show");
                return new CommandInfo(0, 0, commandName);
            }

            case "info": {
                log.info("Сервер получил команду info");
                return new CommandInfo(0, 0, commandName);
            }

            case "insert": {
                log.info("Сервер получил команду insert");
                return new CommandInfo(1, 1, commandName);
            }

            case "exit": {
                log.info("Сервер получил команду exit");
                return new CommandInfo(0, 0, commandName);
            }

            case "help": {
                log.info("Сервер получил команду help");
                return new CommandInfo(0, 0, commandName);
            }

            case "print_descending": {
                log.info("Сервер получил команду print_descending");
                return new CommandInfo(0, 0, commandName);
            }

            case "remove_all_by_color": {
                log.info("Сервер получил команду remove_all_by_color");
                return new CommandInfo(1, 0, commandName);
            }

            case "remove_greater_key": {
                log.info("Сервер получил команду remove_greater_key");
                return new CommandInfo(1, 0, commandName);
            }

            case "remove_key": {
                log.info("Сервер получил команду remove_key");
                return new CommandInfo(1, 0, commandName);
            }
            ...
            case "sum_of_age": {
                log.info("Сервер получил команду sum_of_age");
                return new CommandInfo(0, 0, commandName);
            }


            case "remove_lower": {
                log.info("Сервер получил команду remove_lower");
                return new CommandInfo(0, 1, commandName);
            }

            case "replace_if_lower": {
                log.info("Сервер получил команду replace_if_lower");
                return new CommandInfo(1, 1, commandName);
            }

            case "update": {
                log.info("Сервер получил команду update");
                return new CommandInfo(1, 1, commandName);

            }
            case "execute_script": {
                log.info("Сервер получил команду execute_script");
                return new CommandInfo(1, 0, commandName);
            }
            default: {
                log.info("Сервер получил неизвестную команду");
    ...
    public Command getCommand(CommandInfo commandInfo, CommandArguments commandArguments, Scannable scannable, Printable printable) throws Exception {
        try {
            switch (commandInfo.getName()) {
                ...
                default: {
                    log.error("Некорректно введена команда");
    ...
    public static CommandArguments getCommandArguments(String commandLine, CommandInfo commandInfo, Scannable scannable, boolean isConsole) throws Exception {
        ...
        try {
            ...
            switch (commandInfo.getName()) {
                case "clear": {
                    if (arrayLine.length > commandInfo.getSimpleArguments() + 1) {
                        log.error("Некорректно введена команда clear");
                ...
                case "show": {
                    if (arrayLine.length > commandInfo.getSimpleArguments() + 1) {
                        log.error("Некорректно введена команда show");
                        throw new CollectionException("Команда введена некорректно");
                    }
                ...
                case "info": {
                    if (arrayLine.length > commandInfo.getSimpleArguments() + 1) {
                        log.error("Некорректно введена команда info");
                        throw new CollectionException("Команда введена некорректно");
                    }
                ...
                case "insert": {
                    ...
                    if (arrayLine.length < commandInfo.getSimpleArguments() + 1) {
                        log.error("Некорректно введена команда insert");
                ...
                case "exit": {
                    if (arrayLine.length > commandInfo.getSimpleArguments() + 1) {
                        log.error("Некорректно введена команда exit");
                        throw new CollectionException("Команда введена некорректно");
                    }
                ...
                case "help": {
                    if (arrayLine.length > commandInfo.getSimpleArguments() + 1) {
                        log.error("Некорректно введена команда help");
                        throw new CollectionException("Команда введена некорректно");
                    }
                ...
                case "print_descending": {
                    if (arrayLine.length > commandInfo.getSimpleArguments() + 1) {
                        log.error("Некорректно введена команда print_descending");
                        throw new CollectionException("Команда введена некорректно");
                    }
                ...
                case "remove_all_by_color": {
                    ...
                    if (arrayLine.length < commandInfo.getSimpleArguments() + 1) {
                        log.error("Некорректно введены поля команды remove_all_by_color");
                        throw new CollectionException("Введены не все поля");
                    }
                ...
                case "remove_greater_key": {
                    ...
                    if (arrayLine.length < commandInfo.getSimpleArguments() + 1) {
                        log.error("Некорректно введены поля команды remove_greater_key");
                        throw new CollectionException("Введены не все поля");
                    }
                    Integer key = Integer.parseInt(arrayLine[1]);
                ...
                case "remove_key": {
                    ...
                    if (arrayLine.length < commandInfo.getSimpleArguments() + 1) {
                        log.error("Некорректно введены поля команды remove_key");
                        throw new CollectionException("Введены не все поля");
                    }
                    Integer key = Integer.parseInt(arrayLine[1]);
                ...
                case "save": {
                    if (arrayLine.length > commandInfo.getSimpleArguments() + 1) {
                        log.error("Некорректно введена команда save");
                        throw new CollectionException("Команда введена некорректно");
                    }
                ...
                case "sum_of_age": {
                    if (arrayLine.length > commandInfo.getSimpleArguments() + 1) {
                        log.error("Некорректно введена команда sum_of_age");
                        throw new CollectionException("Команда введена некорректно");
                    }
                ...
                case "remove_lower": {
                    if (arrayLine.length > commandInfo.getSimpleArguments() + 1) {
                        log.error("Некорректно введена команда remove_lower");
                        throw new CollectionException("Команда введена некорректно");
                    }
                    Dragon dragon = new DragonBuilder(isConsole).build(scannable, new ConsolePrint());
                ...
                case "replace_if_lower": {
                    ...
                    if (arrayLine.length < commandInfo.getSimpleArguments() + 1) {
                        log.error("Некорректно введены поля команды replace_if_lower");
                        throw new CollectionException("Введены не все поля");
                    }
                    Dragon dragon = new DragonBuilder(isConsole).build(scannable, new ConsolePrint());
                    Integer key = Integer.parseInt(arrayLine[1]);
                ...
                case "update": {
                    if (arrayLine.length < commandInfo.getSimpleArguments() + 1) {
                        log.error("Некорректно введены поля команды update");
                        throw new CollectionException("Введены не все поля");
                    }
                    Dragon dragon = new DragonBuilder(isConsole).build(scannable, new ConsolePrint());
                ...
                case "execute_script": {
                    ...
                    if (arrayLine.length < commandInfo.getSimpleArguments() + 1) {
                        log.error("Некорректно введены поля команды execute script");
                        throw new CollectionException("Введены не все поля");
                    }
            ...
            log.error("Ошибка при чтении команды");
            throw new ServerException();
        }
        catch (Exception e) {
            log.error("Проблемы с коллекцией");
            throw new CollectionException("Ошибка на строчке " + scannable.linesCount() + ": " + e.getMessage());
        }
    }
    ...
    public Command getCommandFromFile(Scannable fileScan, Printable serverPrint) throws Exception {
        ...
        if (!commandInfo.isStatus()){
            log.error("Ошибка при парсинге команды " + words[0]);
